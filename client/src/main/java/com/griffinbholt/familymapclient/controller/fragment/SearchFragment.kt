...
class SearchFragment : Fragment() {
    ...
    private fun runSearchQuery(textQuery: String) : SearchItemAdapter {
        ...
        val peopleFound: List<ClientPerson> = DataCache.searchPeople()
        val eventsFound: List<ClientEvent> = DataCache.searchEvents()

        return SearchItemAdapter(peopleFound, eventsFound)
    }

    private inner class SearchItemAdapter(private val peopleFound: List<ClientPerson>,
    ...
    private inner class SearchItemHolder(itemView: View,
        ...
        fun bind(person: ClientPerson) {
            ...
            setFirstTextField(person.fullName())
            setSecondTextField(null)
            setIcon(IconGenerator.getGenderIcon(context, person.gender))
        }

        fun bind(event: ClientEvent) {
            this.event = event

            setFirstTextField(event.description())
            setSecondTextField(event.person!!.fullName())
            setIcon(IconGenerator.getMapMarkerIcon(context, event.eventType))
        }

        private fun setFirstTextField(text: String) { firstTextField.text = text }

        private fun setSecondTextField(text: String?) { secondTextField.text = text }

        private fun setIcon(iconDrawable: IconDrawable) { icon.setImageDrawable(iconDrawable) }
